...
Contains:
    Classes
    =======
    FIFOCache - A caching system that implements the
...
class FIFOCache(BaseCaching):
    ...
    already, the first one inserted out of the four is removed.
    """
        ...
        if key and item:
            if key in self.cache_data:
                ...
                if len(self.cache_data) == BaseCaching.MAX_ITEMS:
                    first_key = self.keys_by_insertion_order[0]
                    print("DISCARD: {}".format(first_key))
                    del self.cache_data[first_key]
                    self.keys_by_insertion_order.pop(0)
                self.cache_data[key] = item
